...
import com.dkd.manger.domain.dto.TaskDetailsDto;
...
import com.dkd.manger.mapper.TaskMapper;
...
public class TaskServiceImpl implements ITaskService 
{
     ...
     * @param productTypeId 任务的类型，决定任务的性质（投放、维修、补货、撤机）。
     */
    private void checkTask(Long vmStatus,Long productTypeId) {
        // 如果是投放工单，且设备状态为运行中，则抛出异常，因为设备已在运营中无法进行投放
        if (vmStatus.equals(DkdContants.VM_STATUS_RUNNING) && productTypeId.equals(DkdContants.TASK_TYPE_DEPLOY)){
            throw new RuntimeException("设备已在运营中无法进行投放");
        }
        // 如果是维修工单，且设备状态不是运行中，则抛出异常，因为设备不在运营中无法进行维修
        if (!vmStatus.equals(DkdContants.VM_STATUS_RUNNING) && productTypeId.equals(DkdContants.TASK_TYPE_REPAIR)){
            throw new RuntimeException("设备不在运营中无法进行维修");
        }
        // 如果是补货工单，且设备状态不是运行中，则抛出异常，因为设备不在运营状态无法进行补货
        if (!vmStatus.equals(DkdContants.VM_STATUS_RUNNING) && productTypeId.equals(DkdContants.TASK_TYPE_SUPPLY)){
            throw new RuntimeException("设备不在运营状态无法进行补货");
        }
        // 如果是撤机工单，且设备状态不是运行中，则抛出异常，因为设备不在运营状态无法进行撤机
        if (!vmStatus.equals(DkdContants.VM_STATUS_RUNNING) && productTypeId.equals(DkdContants.TASK_TYPE_REVOKE)){
